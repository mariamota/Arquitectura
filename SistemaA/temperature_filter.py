        ...
        while True:
            try:
                ...
                if idMeasurement == 4:
                    ...
                    c=(doubleValue-32)*5/9                                                                                        
                    d=struct.pack('d', c) 
                    v=struct.unpack("@Q", d)[0] 
                    print "ID = {0} Temperatura {1} Celsius {2} medida {3} unpack {4} ".format(idMeasurement, doubleValue, c, measurement, v)                                                       
                    dataByteTmp=0
                    j=7 
                    while j >= 0:
                ...
                self.closePorts()
                print "{0}::TemperatureFilter; bytes read: {1}, bytes written:{2}".format(self.getName(), bytesRead, byteswritten)
                break